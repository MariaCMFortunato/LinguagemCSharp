...
namespace GestaoFormandos02
{
    internal class DBConnect
    {
        ...
        {
            string query = "select * from vFormadorAreaUser order by nome;";
            ...
            try
            {
                if (OpenConnection())
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        dgv.Rows[idxLinha].Cells["utilizador"].Value = dr[6].ToString();
                        idxLinha++;
                    }
                }
            }
        ...
        public void PreencherDGVFormadorFiltrado(ref DataGridView dgv, string nome, string id_area)
        {
            string query = "select id_formador, nome, nif, area, nome_utilizador from vFormadorAreaUser ";

            if (nome.Length > 0 && id_area.Length >0)
            {
                query = query + " where nome like '" + nome + "' and id_area = '" + id_area + "' ";
            }
            else if (nome.Length > 0)
            {
                query = query + " where nome like '" + nome + "' ";
            ...
            try
            {
                if (OpenConnection())
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        dgv.Rows[idxLinha].Cells["utilizador"].Value = dr[4].ToString();
                        idxLinha++;
                    }
                }
            }
        ...
        public bool UpdateNacionalidade(string idNacional, string alf2, string nacionalidade)
        {
            bool flag = true;

            string query = "update nacionalidade set alf2 = '" + alf2 + "', nacionalidade = '" + 
                nacionalidade + "' where id_nacionalidade = '" + idNacional + "';";
