...
namespace GestaoFormacao_PF
{
    public partial class FormPrincipal : Form
    {
        ...
        FormApagarAcao formApagarAcao = new FormApagarAcao();
        FormListarAcoes formListarAcoes = new FormListarAcoes();
        FormInserirTurma formInserirTurma = new FormInserirTurma();
        FormInserirModAcao formInserirModAcao = new FormInserirModAcao();
        FormAtualizarTurma formAtualizarTurma = new FormAtualizarTurma();
        FormAtualizarModAcao formAtualizarModAcao = new FormAtualizarModAcao();
        ...
        private void apagarAçõesFormaçãoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (formApagarAcao.IsDisposed)
            {
                formApagarAcao = new FormApagarAcao();
            }
            formApagarAcao.MdiParent = this;
            formApagarAcao.StartPosition = FormStartPosition.Manual;
            formApagarAcao.Location = new Point((this.ClientSize.Width - formApagarAcao.Width) / 2, (this.Height - formApagarAcao.Height) / 2);
            formApagarAcao.Show();
            formApagarAcao.Activate();
        }

        private void listarAçõesFormaçãoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (formListarAcoes.IsDisposed)
            {
                formListarAcoes = new FormListarAcoes();
            }
            formListarAcoes.MdiParent = this;
            formListarAcoes.StartPosition = FormStartPosition.Manual;
            formListarAcoes.Location = new Point((this.ClientSize.Width - formListarAcoes.Width) / 2, (this.Height - formListarAcoes.Height) / 2);
            formListarAcoes.Show();
            formListarAcoes.Activate();
        }

        private void turmaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (formInserirTurma.IsDisposed)
            {
                formInserirTurma = new FormInserirTurma();
            }
            formInserirTurma.MdiParent = this;
            formInserirTurma.StartPosition = FormStartPosition.Manual;
            formInserirTurma.Location = new Point((this.ClientSize.Width - formInserirTurma.Width) / 2, (this.Height - formInserirTurma.Height) / 2);
            formInserirTurma.Show();
            formInserirTurma.Activate();
        }

        private void moduloAçãoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (formInserirModAcao.IsDisposed)
            {
                formInserirModAcao = new FormInserirModAcao();
            }
            formInserirModAcao.MdiParent = this;
            formInserirModAcao.StartPosition = FormStartPosition.Manual;
            formInserirModAcao.Location = new Point((this.ClientSize.Width - formInserirModAcao.Width) / 2, (this.Height - formInserirModAcao.Height) / 2);
            formInserirModAcao.Show();
            formInserirModAcao.Activate();
        }

        private void turmaToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (formAtualizarTurma.IsDisposed)
            {
                formAtualizarTurma = new FormAtualizarTurma();
            }
            formAtualizarTurma.MdiParent = this;
            formAtualizarTurma.StartPosition = FormStartPosition.Manual;
            formAtualizarTurma.Location = new Point((this.ClientSize.Width - formAtualizarTurma.Width) / 2, (this.Height - formAtualizarTurma.Height) / 2);
            formAtualizarTurma.Show();
            formAtualizarTurma.Activate();
        }

        private void moduloAçãoToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (formAtualizarModAcao.IsDisposed)
            {
                formAtualizarModAcao = new FormAtualizarModAcao();
            }
            formAtualizarModAcao.MdiParent = this;
            formAtualizarModAcao.StartPosition = FormStartPosition.Manual;
            formAtualizarModAcao.Location = new Point((this.ClientSize.Width - formAtualizarModAcao.Width) / 2, (this.Height - formAtualizarModAcao.Height) / 2);
            formAtualizarModAcao.Show();
            formAtualizarModAcao.Activate();
        }
    }
}
